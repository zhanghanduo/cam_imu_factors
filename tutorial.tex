...
\paragraph{States}
The state vector to be estimated contains the 3-D body position $\bm{p}^{\mathrm{w}}$, velocity $\bm{v}^{\mathrm{w}}$ an`d rotations in form of %quaternion $\bm{q}^{\mathrm{w}}$ (converted from 
...
%With a stereo camera, Todd Lupton also provided a way to recover the absolute global frame after 3 images of observation. However this is not possible with monocular camera setup.
